...
def test_surface_impact_event_terminates_inbound_trajectory() -> None:
    ...
    assert np.linalg.norm(impact_state[:3]) == pytest.approx(GRGM1200A_J2.collision_radius_m, abs=1e-3)
...
def test_default_integration_converges_to_tighter_reference() -> None:
    ...
    tighter = propagate(
        ...
        settings=PropagationSettings(rtol=1e-13, position_atol_m=1e-9, velocity_atol_m_s=1e-12),
    )
...
def test_generic_acceleration_propagator_matches_two_body_wrapper() -> None:
    from lunar_astrodynamics import central_acceleration, propagate_with_acceleration

    elements = _validation_orbit()
    state0 = state_from_elements(elements, GRGM1200A_J2.mu_m3_s2)
    period = orbital_period_s(elements.semi_major_axis_m, GRGM1200A_J2.mu_m3_s2)
    times = np.linspace(0.0, 2.0 * period, 51)
    reference = propagate(state0, times[-1], include_j2=False, sample_times_s=times)
    generic = propagate_with_acceleration(
        state0,
        times[-1],
        lambda _t, r: central_acceleration(r, GRGM1200A_J2.mu_m3_s2),
