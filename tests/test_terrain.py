...
def test_gridline_bilinear_interpolation_matches_known_surface() -> None:
    ...
    # Interior bilinear interpolation, away from the periodic seam.
...
def test_exact_pole_value_is_independent_of_longitude() -> None:
    ...
    assert values == pytest.approx([100.25] * 4)
...
def test_pixel_registered_polar_cap_extrapolates_to_unique_pole() -> None:
    ...
    # Halfway from the last center ring to the pole at longitude 45 deg.
...
def test_gmt_netcdf_loader_reads_gridline_coordinates_and_elevation(tmp_path: Path) -> None:
    path = tmp_path / "synthetic.grd"
    lon = np.array([0.0, 90.0, 180.0, 270.0, 360.0], dtype=np.float64)
    lat = np.array([-90.0, -45.0, 0.0, 45.0, 90.0], dtype=np.float64)
    z = np.add.outer(lat, lon / 10.0).astype(np.float32)
    z[:, -1] = z[:, 0]
...
def test_pds_global_gdr_loader_uses_label_scaling_and_pixel_centers(tmp_path: Path) -> None:
    ...
    # File row zero is the northernmost pixel. After loader reversal this is
    # latitude +89.5 degrees, longitude +0.5 degrees.
    raw[0, 0] = 2468
    raw[-1, -1] = -200
